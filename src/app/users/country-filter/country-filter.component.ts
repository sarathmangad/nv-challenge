import { Component, OnInit, ViewChild, EventEmitter, Output } from '@angular/core';

@Component({
  selector: 'app-country-filter',
  templateUrl: './country-filter.component.html',
  styleUrls: ['./country-filter.component.css']
})
export class CountryFilterComponent implements OnInit {

  constructor() { }
  selectedCountries;
  @Output() selectedCountryChange = new EventEmitter();

  ngOnInit() {
    this.selectedCountries = []
  }

  getCountryList(){
    return COUNTRY_LIST
  }

  getAlphabets(){
    return Object.keys(this.getCountryList())
  }

  onClickCountry(country){
    var index = this.selectedCountries.indexOf(country) ;
    if(index == -1)
      this.addToSelectedList(country)
    else
      this.removeFromSelectedList(index)
  }
  addToSelectedList(country){
      this.selectedCountries.push(country)
      this.selectedCountryChange.emit(this.selectedCountries)
  }
  removeFromSelectedList(index){
      this.selectedCountries.splice(index, 1)
      this.selectedCountryChange.emit(this.selectedCountries)
  }

}

const COUNTRY_LIST = {
    "A":{
      "countries":[
        {
          "country":"Afghanistan"
        },
        {
          "country":"Albania"
        },
        {
          "country":"Algeria"
        },
        {
          "country":"Andorra"
        },
        {
          "country":"Angola"
        },
        {
          "country":"Antigua and Barbuda"
        },
        {
          "country":"Argentina"
        },
        {
          "country":"Armenia"
        },
        {
          "country":"Australia"
        },
        {
          "country":"Austria"
        },
        {
          "country":"Azerbaijan"
        }
      ]
    },
    "B":{
      "countries":[
        {
          "country":"Bahamas"
        },
        {
          "country":"Bahrain"
        },
        {
          "country":"Bangladesh"
        },
        {
          "country":"Barbados"
        },
        {
          "country":"Belarus"
        },
        {
          "country":"Belgium"
        },
        {
          "country":"Belize"
        },
        {
          "country":"Benin"
        },
        {
          "country":"Bhutan"
        },
        {
          "country":"Bolivia"
        },
        {
          "country":"Bosnia and Herzegovina"
        },
        {
          "country":"Botswana"
        },
        {
          "country":"Brazil"
        },
        {
          "country":"Brunei"
        },
        {
          "country":"Bulgaria"
        },
        {
          "country":"Burkina Faso"
        },
        {
          "country":"Burundi"
        }
      ]
    },
    "C":{
      "countries":[
        {
          "country":"Cabo Verde"
        },
        {
          "country":"Cambodia"
        },
        {
          "country":"Cameroon"
        },
        {
          "country":"Canada"
        },
        {
          "country":"Central African Republic"
        },
        {
          "country":"Chad"
        },
        {
          "country":"Chile"
        },
        {
          "country":"China"
        },
        {
          "country":"Colombia"
        },
        {
          "country":"Comoros"
        },
        {
          "country":"Congo, Republic of the"
        },
        {
          "country":"Congo, Democratic Republic of the"
        },
        {
          "country":"Costa Rica"
        },
        {
          "country":"Ivory Coast"
        },
        {
          "country":"Croatia"
        },
        {
          "country":"Cuba"
        },
        {
          "country":"Cyprus"
        },
        {
          "country":"Czech Republic"
        }
      ]
    },
    "D":{
      "countries":[
        {
          "country":"Denmark"
        },
        {
          "country":"Djibouti"
        },
        {
          "country":"Dominica"
        },
        {
          "country":"Dominican Republic"
        }
      ]
    },
    "E":{
      "countries":[
        {
          "country":"Ecuador"
        },
        {
          "country":"Egypt"
        },
        {
          "country":"El Salvador"
        },
        {
          "country":"Equatorial Guinea"
        },
        {
          "country":"Eritrea"
        },
        {
          "country":"Estonia"
        },
        {
          "country":"Ethiopia"
        }
      ]
    },
    "F":{
      "countries":[
        {
          "country":"Fiji"
        },
        {
          "country":"Finland"
        },
        {
          "country":"France"
        }
      ]
    },
    "G":{
      "countries":[
        {
          "country":"Gabon"
        },
        {
          "country":"Gambia"
        },
        {
          "country":"Georgia"
        },
        {
          "country":"Germany"
        },
        {
          "country":"Ghana"
        },
        {
          "country":"Greece"
        },
        {
          "country":"Grenada"
        },
        {
          "country":"Guatemala"
        },
        {
          "country":"Guinea"
        },
        {
          "country":"Guinea-Bissau"
        },
        {
          "country":"Guyana"
        }
      ]
    },
    "H":{
      "countries":[
        {
          "country":"Haiti"
        },
        {
          "country":"Honduras"
        },
        {
          "country":"Hungary"
        }
      ]
    },
    "I":{
      "countries":[
        {
          "country":"Iceland"
        },
        {
          "country":"India"
        },
        {
          "country":"Indonesia"
        },
        {
          "country":"Iran"
        },
        {
          "country":"Iraq"
        },
        {
          "country":"Ireland"
        },
        {
          "country":"Israel"
        },
        {
          "country":"Italy"
        }
      ]
    },
    "J":{
      "countries":[
        {
          "country":"Jamaica"
        },
        {
          "country":"Japan"
        },
        {
          "country":"Jordan"
        }
      ]
    },
    "K":{
      "countries":[
        {
          "country":"Kazakhstan"
        },
        {
          "country":"Kenya"
        },
        {
          "country":"Kiribati"
        },
        {
          "country":"Kosovo"
        },
        {
          "country":"Kuwait"
        },
        {
          "country":"Kyrgyzstan"
        }
      ]
    },
    "L":{
      "countries":[
        {
          "country":"Laos"
        },
        {
          "country":"Latvia"
        },
        {
          "country":"Lebanon"
        },
        {
          "country":"Lesotho"
        },
        {
          "country":"Liberia"
        },
        {
          "country":"Libya"
        },
        {
          "country":"Liechtenstein"
        },
        {
          "country":"Lithuania"
        },
        {
          "country":"Luxembourg"
        }
      ]
    },
    "M":{
      "countries":[
        {
          "country":"Macedonia"
        },
        {
          "country":"Madagascar"
        },
        {
          "country":"Malawi"
        },
        {
          "country":"Malaysia"
        },
        {
          "country":"Maldives"
        },
        {
          "country":"Mali"
        },
        {
          "country":"Malta"
        },
        {
          "country":"Marshall Islands"
        },
        {
          "country":"Mauritania"
        },
        {
          "country":"Mauritius"
        },
        {
          "country":"Mexico"
        },
        {
          "country":"Micronesia"
        },
        {
          "country":"Moldova"
        },
        {
          "country":"Monaco"
        },
        {
          "country":"Mongolia"
        },
        {
          "country":"Montenegro"
        },
        {
          "country":"Morocco"
        },
        {
          "country":"Mozambique"
        },
        {
          "country":"Myanmar (Burma)"
        }
      ]
    },
    "N":{
      "countries":[
        {
          "country":"Namibia"
        },
        {
          "country":"Nauru"
        },
        {
          "country":"Nepal"
        },
        {
          "country":"Netherlands"
        },
        {
          "country":"New Zealand"
        },
        {
          "country":"Nicaragua"
        },
        {
          "country":"Niger"
        },
        {
          "country":"Nigeria"
        },
        {
          "country":"North Korea"
        },
        {
          "country":"Norway"
        }
      ]
    },
    "O":{
      "countries":[
        {
          "country":"Oman"
        }
      ]
    },
    "P":{
      "countries":[
        {
          "country":"Pakistan"
        },
        {
          "country":"Palau"
        },
        {
          "country":"Palestine"
        },
        {
          "country":"Panama"
        },
        {
          "country":"Papua New Guinea"
        },
        {
          "country":"Paraguay"
        },
        {
          "country":"Peru"
        },
        {
          "country":"Philippines"
        },
        {
          "country":"Poland"
        },
        {
          "country":"Portugal"
        }
      ]
    },
    "Q":{
      "countries":[
        {
          "country":"Qatar"
        }
      ]
    },
    "R":{
      "countries":[
        {
          "country":"Romania"
        },
        {
          "country":"Russia"
        },
        {
          "country":"Rwanda"
        }
      ]
    },
    "S":{
      "countries":[
        {
          "country":"St. Kitts and Nevis "
        },
        {
          "country":"St. Lucia"
        },
        {
          "country":"St. Vincent and The Grenadines "
        },
        {
          "country":"Samoa"
        },
        {
          "country":"San Marino"
        },
        {
          "country":"Sao Tome and Principe "
        },
        {
          "country":"Saudi Arabia"
        },
        {
          "country":"Senegal"
        },
        {
          "country":"Serbia"
        },
        {
          "country":"Seychelles"
        },
        {
          "country":"Sierra Leone"
        },
        {
          "country":"Singapore"
        },
        {
          "country":"Slovakia"
        },
        {
          "country":"Slovenia"
        },
        {
          "country":"Solomon Islands"
        },
        {
          "country":"Somalia"
        },
        {
          "country":"South Africa"
        },
        {
          "country":"South Korea"
        },
        {
          "country":"South Sudan"
        },
        {
          "country":"Spain"
        },
        {
          "country":"Sri Lanka"
        },
        {
          "country":"Sudan"
        },
        {
          "country":"Suriname"
        },
        {
          "country":"Swaziland"
        },
        {
          "country":"Sweden"
        },
        {
          "country":"Switzerland"
        },
        {
          "country":"Syria"
        }
      ]
    },
    "T":{
      "countries":[
        {
          "country":"Taiwan"
        },
        {
          "country":"Tajikistan"
        },
        {
          "country":"Tanzania"
        },
        {
          "country":"Thailand"
        },
        {
          "country":"Timor-Leste"
        },
        {
          "country":"Togo"
        },
        {
          "country":"Tonga"
        },
        {
          "country":"Trinidad and Tobago"
        },
        {
          "country":"Tunisia"
        },
        {
          "country":"Turkey"
        },
        {
          "country":"Turkmenistan"
        },
        {
          "country":"Tuvalu"
        }
      ]
    },
    "U":{
      "countries":[
        {
          "country":"Uganda"
        },
        {
          "country":"Ukraine"
        },
        {
          "country":"United Arab Emirates"
        },
        {
          "country":"United Kingdom (UK)"
        },
        {
          "country":"United States of America (USA)"
        },
        {
          "country":"Uruguay"
        },
        {
          "country":"Uzbekistan"
        }
      ]
    },
    "V":{
      "countries":[
        {
          "country":"Vanuatu"
        },
        {
          "country":"Vatican City (Holy See)"
        },
        {
          "country":"Venezuela"
        },
        {
          "country":"Vietnam"
        }
      ]
    },
    "W":{
      "countries":[
        {
          "country":"Yemen"
        }
      ]
    },
    "Z":{
      "countries":[
        {
          "country":"Zambia"
        },
        {
          "country":"Zimbabwe"
        }
      ]
    }
  }
